...
namespace netket {
	...
	public:
		...
		MPSDiagonal(const Hilbert &hilbert, const int &N, const int &D, const int &symperiod)
		...
		inline VectorType mps_contraction(const Eigen::VectorXd &v,
			...
			for (std::size_t k = 0; k < nconn; k++) {
				...
				if (nchange > 0) {
					...
					if (site == 0) {
						...
						new_prods = mps_contraction(v, 0, site).cwiseProduct(W_[site % symperiod_][confindex_[newconf[k][sorted_ind[0]]]]);
					}

					for (std::size_t i = 1; i < nchange; i++) {
						site = tochange[k][sorted_ind[i]];
						new_prods = new_prods.cwiseProduct(mps_contraction(v, tochange[k][sorted_ind[i - 1]] + 1, site).cwiseProduct(W_[site % symperiod_][confindex_[newconf[k][sorted_ind[i]]]]));
					}
		...
		T LogValDiff(const Eigen::VectorXd &v, const std::vector<int> &toflip,
			...
			if (site == 0) {
				...
				new_prod = lt.V(2 * (site - 1)).cwiseProduct(W_[site % symperiod_][confindex_[newconf[sorted_ind[0]]]]);
			}

			for (std::size_t k = 1; k < nflip; k++) {
				site = toflip[sorted_ind[k]];
				new_prod = new_prod.cwiseProduct(mps_contraction(v, toflip[sorted_ind[k - 1]] + 1, site).cwiseProduct(W_[site % symperiod_][confindex_[newconf[sorted_ind[k]]]]));
			}
		...
		VectorType DerLog(const Eigen::VectorXd &v) override {
			...
			for (int site = 1; site < N_- 1; site++) {
				left_prods.push_back(left_prods[site - 1].cwiseProduct(W_[site % symperiod_][confindex_[v(site)]]));
				right_prods.push_back(W_[(N_ - 1 - site) % symperiod_][confindex_[v(N_ - 1 - site)]].cwiseProduct(right_prods[site-1]));
			}
			left_prods.push_back(left_prods[N_ -2].cwiseProduct(W_[(N_-1) % symperiod_][confindex_[v(N_ -1)]]));
			right_prods.push_back(W_[0][confindex_[v(0)]].cwiseProduct(right_prods[N_ - 2]));

			der.segment(confindex_[v(0)] * D_, D_) += Eigen::Map<VectorType>((right_prods[N_ - 2]).transpose().data(), D_);
			for (int site = 1; site < N_ - 1; site++) {
