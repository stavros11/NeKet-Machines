...
class MPSOpen : public AbstractMachine<T> {
 ...
 public:
  ...
  void Init() {
    ...
    InfoMessage() << "Open MPS machine with " << N_ << " sites created"
                  << std::endl;
    InfoMessage() << "Physical dimension d = " << d_
                  << " and bond dimensions D = ";
  ...
  // inline void ComputeVtilde(const Eigen::VectorXd &v, Eigen::VectorXd
  // &vtilde) { 	vtilde = (v + Eigen::VectorXd::Ones(N_)) / 2;
  //}
  ...
  void normalize2canonical() {
    ...
    for (int i = d_ * (N_ - 1); i < d_ * N_; i++) {
      W_[i] = SdotV * W_[i];
      //	last_norm +=
      // std::real((W_[i].conjugate().cwiseProduct(W_[i]).sum()));
    }
  ...
  inline MatrixType list2mat(const int site, const MatrixType SdotV) {
    ...
    for (int i = 0; i < d_; i++) {
      mat.block(i * D_[site], 0, D_[site], D_[site + 1]) =
          SdotV * W_[site * d_ + i];
    }
    return mat;
  };
  ...
  void InitLookup(const Eigen::VectorXd &v, LookupType &lt) override {
    ...
    lt.M(2 * N_ - 2) =
        lt.M(2 * N_ - 4) * W_[d_ * (N_ - 1) + confindex_[v(N_ - 1)]];
  ...
  // Auxiliary function
  inline void _InitLookupLeft_check(LookupType &lt, const int i,
                                    const int Dright) {
    if (lt.MatrixSize() == i) {
  ...
  // Auxiliary function
  inline void _InitLookupRight_check(LookupType &lt, const int i,
                                     const int Dleft) {
    if (lt.MatrixSize() == i) {
  ...
  // Auxiliary function that calculates all left and right contractions (not
  // ready yet)
  /**
  inline std::vector<MatrixType> initialize_left_right(const Eigen::VectorXd
  &vtilde) { int site; std::vector<MatrixType> prods;
          ...
          for (int i = 2; i < 2 * N_; i += 2) {
                  site = i / 2;
                  prods.push_back(prods[i - 2] * W_[d_ * site +
  (int)vtilde(site)]); prods.push_back(W_[(int)vtilde(site) * prods[i - 2]]);
          }
          return prods;
  }; */
  ...
  void UpdateLookup(const Eigen::VectorXd &v, const std::vector<int> &tochange,
    ...
    if (site == 0) {
      ...
      lt.M(2 * site) = lt.M(2 * (site - 1)) *
                       W_[d_ * site + confindex_[newconf[sorted_ind[0]]]];
    ...
    for (std::size_t k = 1; k < nchange; k++) {
      for (site = tochange[sorted_ind[k - 1]] + 1;
        ...
        lt.M(2 * site) =
            lt.M(2 * (site - 1)) * W_[d_ * site + confindex_[v(site)]];
        // loc_lt[2 * site] = lt.M(2 * site);
      }
      site = tochange[sorted_ind[k]];
      lt.M(2 * site) = lt.M(2 * (site - 1)) *
                       W_[d_ * site + confindex_[newconf[sorted_ind[k]]]];
      // loc_lt[2 * site] = lt.M(2 * site);
    }
    ...
    if (site == N_ - 1) {
      lt.M(1) =
          W_[d_ * (N_ - 1) + confindex_[newconf[sorted_ind[nchange - 1]]]];
    } else {
      lt.M(2 * (N_ - site) - 1) =
          W_[d_ * site + confindex_[newconf[sorted_ind[nchange - 1]]]] *
          lt.M(2 * (N_ - site) - 3);
    }
    ...
    for (int k = nchange - 2; k >= 0; k--) {
      for (site = tochange[sorted_ind[k + 1]] - 1;
           site > tochange[sorted_ind[k]]; site--) {
        lt.M(2 * (N_ - site) - 1) =
            W_[d_ * site + confindex_[v(site)]] * lt.M(2 * (N_ - site) - 3);
      }
      site = tochange[sorted_ind[k]];
      lt.M(2 * (N_ - site) - 1) =
          W_[d_ * site + confindex_[newconf[sorted_ind[k]]]] *
    ...
    for (std::size_t k = 0; k < nconn; k++) {
      ...
      if (nchange > 0) {
        ...
        if (site == 0) {
          ...
          new_prods = mps_contractionLeft(v, site) *
                      W_[d_ * site + confindex_[newconf[k][sorted_ind[0]]]];
        }
        for (std::size_t i = 1; i < nchange; i++) {
          site = tochange[k][sorted_ind[i]];
          new_prods *=
              mps_contraction(v, tochange[k][sorted_ind[i - 1]] + 1, site) *
              W_[d_ * site + confindex_[newconf[k][sorted_ind[i]]]];
        }
        site = tochange[k][sorted_ind[nchange - 1]];
        if (site < N_ - 1) {
          logvaldiffs(k) =
              std::log((new_prods * mps_contractionRight(v, site + 1)).trace() /
                       current_psi);
        } else {
  ...
  inline MatrixType mps_contractionLeft(const Eigen::VectorXd &v,
                                        const int &site) {
  ...
  inline MatrixType mps_contractionRight(const Eigen::VectorXd &v,
                                         const int &site) {
  ...
  T LogValDiff(const Eigen::VectorXd &v, const std::vector<int> &toflip,
          ...
          if (toflip[0] == 0) {
                  ...
                  new_prods = mps_contraction(vtilde_, 0, toflip[0]) * W_[d_ *
  toflip[0] + confindex_[newconf[0]]];
          }
          for (std::size_t i = 1; i < nflip; i++) {
                  //InfoMessage() << "toflip = " << toflip[i] << std::endl;
                  new_prods *= mps_contraction(vtilde_, toflip[i - 1] + 1,
  toflip[i]) * W_[d_ * toflip[i] + confindex_[newconf[i]]];
          }
          if (toflip[nflip - 1] < N_ - 1) {
                  new_prods *= mps_contraction(vtilde_, toflip[nflip - 1] + 1,
  N_);
          }
          //InfoMessage() << "LogValDiff lookup ended " <<
  std::log(new_prods.trace() / current_psi) << std::endl; return
  std::log(new_prods.trace() / current_psi);
  };*/
  ...
  T LogValDiff(const Eigen::VectorXd &v, const std::vector<int> &toflip,
    ...
    if (site == N_ - 1) {
      return std::log(new_prod.trace() /
                      lt.M(2 * N_ - 2).trace());  // A log is needed here
    } else {
      return std::log((new_prod * lt.M(2 * (N_ - site) - 3)).trace() /
                      lt.M(2 * N_ - 2).trace());  // A log is needed here
    }
  };
  ...
  VectorType DerLog(const Eigen::VectorXd &v) override {
    ...
    for (int site = 1; site < N_ - 1; site++) {
      ...
      left_prods.push_back(left_prods[site - 1] *
                           W_[d_ * site + confindex_[v(site)]]);
      right_prods.push_back(
          W_[d_ * (N_ - 1 - site) + confindex_[v(N_ - 1 - site)]] *
          right_prods[site - 1]);
    }
    left_prods.push_back(left_prods[N_ - 2] *
                         W_[d_ * (N_ - 1) + confindex_[v(N_ - 1)]]);
    right_prods.push_back(W_[confindex_[v(0)]] * right_prods[N_ - 2]);

    // InfoMessage() << "Products calculated" << std::endl;

    der.segment(confindex_[v(0)] * D_[1], D_[1]) =
        Eigen::Map<VectorType>((right_prods[N_ - 2]).data(), D_[1]);
    // InfoMessage() << "Left derivative assigned" << std::endl;
    for (int site = 1; site < N_ - 1; site++) {
      Dsq = D_[site] * D_[site + 1];
      der.segment(d_ * D_[1] + ((site - 1) * d_ + confindex_[v(site)]) * Dsq,
                  Dsq) = middle_tensor_product(left_prods[site - 1],
                                               right_prods[N_ - site - 2]);

      // InfoMessage() << "site = " << site << std::endl;
    }
    der.segment(npar_ + (confindex_[v(N_ - 1)] - d_) * D_[N_ - 1], D_[N_ - 1]) =
        Eigen::Map<VectorType>((left_prods[N_ - 2]).data(), D_[N_ - 1]);
  ...
  inline VectorType middle_tensor_product(const MatrixType left,
  ...
  void from_json(const json &pars) override {
    ...
    if (FieldExists(pars["Machine"], "BondDim")) {
      ...
      // Otherwise treat it as an integer and create D_ with the same BondDim
      // for all sites
      catch (...) {
        D_.push_back(1);
        for (int i = 1; i < N_; i++) {
